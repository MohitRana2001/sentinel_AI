"""
AlloyDB + pgvector utilities for vector storage
Replaces ChromaDB with AlloyDB pgvector
"""
...
class VectorStore:
    """Vector store using AlloyDB with pgvector"""
    ...
    def __init__(self, db: Session):
        ...
        try:
            ...
            print(f"⚠️  Embedding model unavailable ({exc}). Falling back to keyword search.")
    ...
    def add_document_chunks(
        ...
        """
        Chunk text, create embeddings, and store in AlloyDB
        ...
        for idx, chunk_text in enumerate(chunks):
            ...
            if self.embedding_available and self.embeddings is not None:
                try:
                    ...
                    self.embedding_available = False
                    embedding_vector = None
                    print(f"⚠️  Failed to generate embedding ({exc}). Using fallback mode.")
    ...
    def similarity_search(
        ...
        if self.embedding_available and self.embeddings is not None:
            try:
                ...
                self.embedding_available = False
                print(f"⚠️  Failed to embed query ({exc}). Using fallback keyword search.")
                query_embedding = None
        ...
        if query_embedding is not None:
            # Order by similarity (using pgvector's <=> operator)
            # Note: This requires raw SQL for optimal performance
...
def vectorise_and_store_alloydb(
    ...
    """
    Wrapper function compatible with existing document_chunker.py logic
    Stores in AlloyDB instead of ChromaDB
